...
impl Group {
    pub async fn create(pool: &SqlitePool, name: &str, created_by: i64) -> sqlx::Result<i64> {
    ...
    pub async fn find_by_invite_code(pool: &SqlitePool, code: &str) -> sqlx::Result<Option<Self>> {
        sqlx::query_as("SELECT * FROM groups WHERE invite_code = ?")
            .bind(code)
            .fetch_optional(pool)
            .await
    }

    pub async fn join(pool: &SqlitePool, group_id: i64, user_id: i64) -> sqlx::Result<()> {
    ...
    pub async fn user_groups(pool: &SqlitePool, user_id: i64) -> sqlx::Result<Vec<GroupWithMembership>> {
        sqlx::query_as(
            r#"
    ...
    pub async fn member_streaks(pool: &SqlitePool, group_id: i64) -> sqlx::Result<Vec<MemberWithStreaks>> {
        sqlx::query_as(
            r#"
            WITH RECURSIVE streak_cte(task_id, streak_date, streak_count) AS (
                ...
                JOIN tasks t ON t.id = c.task_id
                JOIN group_members gm ON gm.user_id = t.user_id AND gm.group_id = ?1
                WHERE c.completed_date = date('now', '-1 day')
                  AND NOT EXISTS (
                      SELECT 1 FROM completions c2
                      WHERE c2.task_id = c.task_id AND c2.completed_date = date('now')
                  )
                UNION ALL
            ...
            JOIN tasks t ON t.user_id = u.id AND t.archived = 0
